<template>
  ...
  >
    ...
    <div slot="footer" class="dialog-footer">
      ...
      <el-button type="primary" :loading="savebutton" @click="getSysVulnInfo()">确 定</el-button>
    </div>
  </el-dialog>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      title: '',
      tableThead: [// 漏洞明细
        { 'id': '1', 'field': 'cve_id', 'fieldType': 'text', 'text': 'CVE编号', 'align': 'left' },
  ...
  methods: {
    ...
    getSysVulnInfo() {
      ...
      if (!$self.multiple) { // 单选
        if ($self.currentRow == null) {
          ...
          $self.currentRow.cveId = $self.currentRow.cve_id
          this.$emit('getSysVulnInfo', $self.currentRow, this.parentComponentParam)//, this.parentComponentParam
          this.$emit('update:visible', false) // 直接修改父组件的属性
        }
      } else {
        if ($self.multipleSelection == null) {
          ...
          this.$emit('getSysVulnInfo', $self.multipleSelection, this.parentComponentParam)//, this.parentComponentParam
          this.$emit('update:visible', false) // 直接修改父组件的属性
        }
      }
    },
    ...
    getTableData() {
      ...
      this.$store.dispatch('disposal/querySysVulnListByPage', param).then(response => {
        $self.loading = false
