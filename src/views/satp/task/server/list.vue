    ...
    return {
      ...
      taskGrid: {
        ...
        columns: [
          ...
          { type: 'text', label: '编号', prop: 'taskId', width: 50, align: 'center' },
          { type: 'text', label: '来源', prop: 'taskSrc', formatter: this.taskSrcFormatter, width: 50, align: 'center' },
          { type: 'text', label: '执行计划', prop: 'planType', formatter: this.parserTaskPlan, width: 100, align: 'center' },
    ...
    operClick(data) {
      ...
      switch (prop) {
        ...
        case 'start':
          ...
          break
        case 'config':
          path = '/satp-task/server/config/' + row.taskId
          this.$router.push({ path: path })
          break
        case 'detail':
          path = '/satp-task/server/detail/' + row.taskId
          this.$router.push({ path: path })
          break
        case 'report':
          path = '/satp-task/server/report/' + row.taskId
          this.$router.push({ path: path })
          break
        default:
          break
