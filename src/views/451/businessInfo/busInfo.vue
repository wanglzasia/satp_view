<template>
  <div>
    <div class="app-container">
      <div>
        <el-page-header content="业务清单详情" @back="goBack" />
        <router-view />
      </div>
      <el-card>
        <el-form
        ...
        <div :hidden="assesHidden">
          ...
          >
            ...
            <el-row>
              <el-col :span="22">
                <el-form-item>
                  <el-button type="primary" @click="onSubmit">发起评估</el-button>
...
export default {
  ...
  data() {
    return {
      ...
      formRule: {
        bussTtid: [
          { required: true, message: '请选择特通人员', trigger: 'blur' }
        ],
        bussTttypeid: [
          { required: true, message: '请选择网络改造情况', trigger: 'blur' }
        ],
        recordFlag: [
          { required: true, message: '请选择是否报备管局', trigger: 'blur' }
        ],
        serviceUrl: [
          { required: true, message: '请填写业务访问地址', trigger: 'blur' }
        ],
        testTarget: [
          { required: true, message: '请填写测试对象', trigger: 'blur' }
        ],
        testAccount: [
  ...
  created() {
    ...
    this.assesHidden = this.$route.query.assesHidden
    this.assessmentObj.bussId = this.$route.query.bussId
    this.backForm.evalName = this.$route.query.evalName
    this.backForm.createUser = this.$route.query.createUser
    this.backForm.createTime = this.$route.query.createTime
    this.backForm.evalProvince = this.$route.query.evalProvince
    this.backForm.orgName = this.$route.query.orgName
    this.backForm.evalId = this.$route.query.evalId
  },
  mounted() {
    this.moreInfo()
  },
  methods: {
    goBack() {
      this.$router.push({
        ...
        query: {
          'evalName': this.backForm.evalName,
          'createUser': this.backForm.createUser,
          'createTime': this.backForm.createTime,
          'evalProvince': this.backForm.evalProvince,
          'orgName': this.backForm.orgName,
          'evalId': this.backForm.evalId
        }
      })
    },
    onSubmit() {
