    ...
    <div>
      <el-row>
        <el-form ref="elForm" :model="queryArgs" size="medium" label-width="100px">
          ...
          <el-col :span="3">
            <el-form-item label="文档目录" prop="fileType">
            ...
            >
              ...
              <el-table-column fixed="right" label="操作" min-width="150" align="center">
                <template slot-scope="scope">
                  ...
                  <el-button type="danger" icon="el-icon-delete" circle @click.native.prevent="deletePolicyFile(scope.row)" />
  ...
  data() {
    return {
      ...
      tableThead: [
        ...
        { 'id': '3', 'field': 'fileTypeName', 'fieldType': 'text', 'text': '文档目录', 'align': 'left' },
        { 'id': '4', 'field': 'source', 'fieldType': 'text', 'text': '文档来源', 'align': 'left' },
  ...
  methods: {
      ...
      $self.$refs['ruleForm'].validate(valid => {
        if (valid) {
          ...
          if ($self.formAttr.formType === 'add') {
            url = 'docManage/addPolicyFile'
          } else if ($self.formAttr.formType === 'edit') {
            url = 'docManage/updatePolicyFile'
          } else {
    ...
    deletePolicyFile(row) {
      ...
      this.$confirm(`您确认要删除吗?`)
        .then(() => {
          this.$store.dispatch('docManage/deletePolicyFile', policyFile).then(response => {
            $self.getTableData()
            $self.$message({
              message: `删除操作成功！`,
              type: 'success'
    ...
    getTableData() {
      ...
      this.$store.dispatch('docManage/policyFileList', params).then(response => {
    ...
    approval(row) {
      ...
      if (row.auditState === '已审核') {
        ...
        this.$store.dispatch('docManage/updatePolicyFile', policyFile).then(response => {
    ...
    showDialog(row, type) {
      ...
      const policyFile = { id: row.id, isFile: 2 }
      if (type === 'edit') {
        this.$store.dispatch('docManage/findPolicyFile', policyFile).then(response => {
          this.formObj = {}
          // 深克隆
