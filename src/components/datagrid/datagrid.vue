<template>
  <div v-loading="dataGridLoading" class="satp-el-datagrid">
    ...
    <div ref="satpdatagrid">
      ...
      >
        <!-- 多选框 -->
        <el-table-column v-if="checkbox" type="selection" :reserve-selection="reservesel" />
        <!-- 序号 -->
        <el-table-column v-if="idx" type="index" :index="index" />
        ...
        <template v-for="(item, colIdx) in columns">
          ...
          >
            <template slot-scope="scope">
              <el-progress :text-inside="true" :stroke-width="14" :percentage="scope.row[item.prop]==null?0:scope.row[item.prop]" status="success" />
            </template>
          </el-table-column>
          ...
          <el-table-column v-else-if="item.actions && !item.formatter && !item.fixed" :key="colIdx" :label="item.label" :min-width="item.minWidth" :width="item.width" :align="item.align">
            <template slot-scope="scope">
          ...
          <!-- 操作按钮列 静态配置-->
          <!--按钮列固定右侧-->
          <el-table-column v-else-if="item.actions && !item.formatter && item.fixed" :key="colIdx" :fixed="item.fixed" :label="item.label" :min-width="item.minWidth" :width="item.width" :align="item.align">
            <template slot-scope="scope">
              <template v-for="(action, actIdx) in item.actions">
                <template v-if="action.popconfirm">
                  <el-popconfirm :key="actIdx" :title="action.popconfirm" @onConfirm="operClick(scope.row, action.prop)">
