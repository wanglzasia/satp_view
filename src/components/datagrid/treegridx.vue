  ...
  <div v-loading="loading" class="satp-el-datagrid">
    ...
    <div ref="satpdatagrid">
      <el-table
        ...
        :row-key="rowKey"
        :load="loadSubList"
        :row-class-name="handelRowClassName"
        class="satp-grid"
...
export default {
  ...
  methods: {
    ...
    handleSwitch(value, row, prop) {
      this.$emit('handleSwitch', value, { row, prop })
    },
    ...
    handelRowClassName(row) {
      let res = ''
      this.$emit('handelRowClassName', row, val => { res = val })
      return res
    }
  }
}
